...
class TICC:
    ...
    def fit(self, input_file):
        ...
        print(time_series_col_size)
        print(time_series_rows_size)  #行数
        ...
        complete_D_train = self.stack_training_data(times_series_arr, time_series_col_size, num_train_points,
                                                    training_indices)# 输入原始数据、列（特征数）、窗口数量、索引

        row_number = int(time_series_rows_size)#有多少tile就有多少个33、34 这里是将每条序列（35组数据）独立计算，不使相邻序列互相影响
        breaktile_33 = range(33, row_number, 35)
        breaktile_34 = range(34, row_number, 35)#根据窗口大小设置
        break_use_tile = np.append(breaktile_33,breaktile_34)

        complete_D_train = np.delete(complete_D_train, break_use_tile,axis=0)#根据窗口大小，将每条序列 35 34 位置删掉，不参与聚类
        ...
        # Initialization
        # Gaussian Mixture
        gmm = mixture.GaussianMixture(n_components=self.number_of_clusters, covariance_type="full",reg_covar=1e-2)#利用GMM算法初始化族和

        # print(gmm)
        print('b')
        gmm.fit(complete_D_train)
        # print(gmm.fit(complete_D_train))
        print('C')
        ...
        for iters in range(self.maxIters):
            ...
            print(len_train_clusters)#len_train_clusters为complete_D_train每条数据对应的初始化类别
            ...
            opt_res = self.train_clusters(cluster_mean_info, cluster_mean_stacked_info, complete_D_train,
                                          ...
                                          # pool,
                                          train_clusters_arr)
            #complete_D_train为已经按窗口大小分好的原始数据 len_train_clusters为complete_D_train每条数据对应的初始化类别
            #time_series_col_size 列数（特征数）train_clusters_arr为列表，每类包含的数据条
            #输出结果为每个类的MRF阵

            self.optimize_clusters(computed_covariance, len_train_clusters, log_det_values, opt_res,
                                   train_cluster_inverse)
            # print(opt_res)
            # print(train_cluster_inverse)
            ...
            clustered_points,lle_all_points_clusters = self.predict_clusters()
        ...
        if self.compute_BIC:
            bic = computeBIC(self.number_of_clusters, time_series_rows_size, clustered_points, train_cluster_inverse,
                             empirical_covariances)
            return clustered_points, train_cluster_inverse, bic,lle_all_points_clusters
    ...
    def optimize_clusters(self, computed_covariance, len_train_clusters, log_det_values, optRes, train_cluster_inverse):
        for cluster in range(self.number_of_clusters):
            # print("d")
            # print(type(optRes[cluster]))
            if optRes[cluster] is None:
                continue

            if optRes[cluster].all() == None :
    ...
    def stack_training_data(self, Data, n, num_train_points, training_indices):# 输入原始数据、列（特征数）、窗口数量（对应点位数量）、索引
    ...
    def predict_clusters(self, test_data = None):
        ...
        # print(lle_all_points_clusters.shape)
        # np.savetxt('Results25_append.txt', lle_all_points_clusters, fmt='%d', delimiter=',')
        ...
        return clustered_points,lle_all_points_clusters
