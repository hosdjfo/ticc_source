...
def updateClusters(LLE_node_vals, switch_penalty=1):
    ...
    """
    #LLE为之前计算所得，为各个划分块与各个聚类中心的对数似然
    tempbeta = switch_penalty
    print('up cluster')
    # print(LLE_node_vals)
    ...
    for i in range(T-2, -1, -1):
        j = i+1
        if(i%33==0): #去除相邻序列 beta的影响
            # print(i)
            switch_penalty=0
        else:
            # print(tempbeta)
            switch_penalty = tempbeta
            # print(switch_penalty)
...
def computeF1_macro(confusion_matrix, matching, num_clusters):
    ...
    """
    # Permute the matrix columns
    F1_score = np.zeros(num_clusters)
    All_TP = np.zeros(num_clusters)
    All_FP = np.zeros(num_clusters)
    All_FN = np.zeros(num_clusters)
    rand_index_all = np.zeros(num_clusters)
    ARI_all = np.zeros(num_clusters)
    permuted_confusion_matrix = np.zeros([num_clusters, num_clusters])
    for cluster in range(num_clusters):
        matched_cluster = matching[cluster]
        permuted_confusion_matrix[:, cluster] = confusion_matrix[:, matched_cluster]
# Compute the F1 score for every cluster
    for cluster in range(num_clusters):
        ...
        FP = np.sum(permuted_confusion_matrix[:,cluster]) - TP
        FN = np.sum(permuted_confusion_matrix[cluster,:]) - TP
        TN = np.sum(permuted_confusion_matrix)-(TP+FP+FN)
        rand_index = (TP+TN)/(TP+FP+FN+TN)
        ARI = (2*(TP*TN-FN*FP))/((TP+FN)*(FN+TN)+(TP+FP)*(FP+FN))
        precision = TP/(TP + FP)
        recall = TP/(TP + FN)
        f1 = (2 * precision * recall) / (precision + recall)
        All_TP[cluster] = TP
        ...
        All_FN[cluster] = FN
        F1_score[cluster] = f1
        rand_index_all[cluster] = rand_index
        ARI_all[cluster] = ARI
    all_precision = All_TP.sum()/(All_TP.sum()+All_FP.sum())
    all_recall = All_TP.sum()/(All_TP.sum()+All_FN.sum())
    F1_micro = (2 * all_precision * all_recall) / (all_precision + all_recall)
    F1_micro_array = np.full((num_clusters), F1_micro)
    outd = np.array(list(zip(F1_score, rand_index_all,F1_micro_array,ARI_all)))
    return outd
